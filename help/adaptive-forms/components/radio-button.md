...
アダプティブフォームのコアコンポーネントは、セマンティック `<fieldset>` と `<legend>` HTML 要素を使用して、ラジオボタングループのアクセシビリティを向上させます。 これらの要素により、スクリーンリーダーはフォーム構造を正確に解釈し、グループラベルとオプション間の関係を伝えることができます。

ラジオボタン（v2）コンポーネントは、`<fieldset>` 要素内でレンダリングされ、グループラベルは `<legend>` で囲まれ、最初の子として表示されます。 冗長または無効な ARIA 属性が削除され、個々のラベルがそれぞれの入力に正しく関連付けられます。 これにより、スクリーンリーダーは、各ラジオボタンラベルとそのグループコンテキストを明確に読み取ることができます。
