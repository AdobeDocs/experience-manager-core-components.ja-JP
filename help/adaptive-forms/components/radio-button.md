...
アダプティブ Formsのコアコンポーネントは、セマンティック `<fieldset>` と `<legend>` HTMLの要素を使用して、ラジオボタングループのアクセシビリティを向上しています。 これらの要素を使用すると、スクリーンリーダーでフォーム構造が正確に解釈され、グループラベルとそのオプションの間の関係が伝えられます。
