...
また、次のオプションを使用して、ラジオボタン グループのオプションを書式設定することもできます **オプションのリッチテキストを許可**.
...
- **デフォルトのオプション**  – このオプションを使用すると、フォームの読み込み時に事前選択されたデフォルト値を追加できます。 **送信された値のデータタイプ**&#x200B;が `Number` に設定されていて、「**デフォルトのオプション**」に文字列データを追加した場合、画面には `Value type mismatch` エラーメッセージが表示されます。
