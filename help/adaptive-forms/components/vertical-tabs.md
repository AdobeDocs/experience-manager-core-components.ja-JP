...
アダプティブフォーム内の縦組みタブとは、フォームの複数のセクションがグループ化され、縦組み合わせで個別のタブとして表示されるデザインパターンを指します。ユーザーはタブを切り替えて、フォームの様々なセクションにアクセスできます。各タブは、関連するフォームコンテンツを表示および非表示にするトリガーとして機能します。縦組みタブは、長いフォームを管理可能なセクションに整理し、ユーザーエクスペリエンスを向上させるのに役立ちます。タブは、キーボードのナビゲーションを使用してセクションを切り替えることができるため、障害を持つユーザーがフォームにアクセスしやすくするのに役立ちます。
ユーザーがタブをクリックすると、フォームコンテンツが動的に更新され、対応するセクションが表示されます。
...
| --- | --- |
| v1 | <br>[リリース 2.0.18](/help/adaptive-forms/version.md) 以降と互換性あり\|Compatible\|Compatible |
...
繰り返しオプションを使用すると、垂直タブコンポーネントとその子コンポーネントのレプリケーション、最小繰り返し回数と最大繰り返し回数の定義、フォーム内の類似セクションのレプリケーションを簡単に行うことができます。 垂直タブコンポーネントを操作してその設定にアクセスすると、次のオプションが表示されます。
...
デザインダイアログは、コンポーネントの CSS スタイルを定義および管理するために使用します。 アダプティブフォームの垂直タブコアコンポーネントは、AEM の[スタイルシステム](/help/get-started/authoring.md#component-styling)をサポートしています。
