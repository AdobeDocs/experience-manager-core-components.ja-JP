...
デザインダイアログでは、テンプレート作成者がデフォルトでの表示方法を制御できます。アダプティブ Formsの垂直タブコンポーネントでは、次の設定を行うことができます。

- フォーム作成者がアダプティブFormsエディターで垂直タブに追加できるコアコンポーネント
- スタイル（CSS クラス）のシンプルな名前。アダプティブFormsエディターの「垂直タブ」コンポーネントの「プロパティ」ダイアログで適用できます。
