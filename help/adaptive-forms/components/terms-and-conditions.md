...
アダプティブフォームの利用条件コアコンポーネントは、Cloud Service 向けコアコンポーネント 2.0.62 および AEM 6.5.16.0 Forms 以降向けコアコンポーネント 1.1.28 の一部として、2023年2月にリリースされました。次の表に、サポートされているすべてのバージョン、AEM の互換性、対応するドキュメントへのリンクを示します。
