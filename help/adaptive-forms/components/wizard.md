...
しかし、オーディエンスがフォームに入力するにはいくつかのページを経由する必要があるため、フォームの複雑さが増す可能性があります。そのため、ウィザードレイアウトを使用する前に、フォームの要件とユーザーのニーズを評価する必要があります。
アダプティブフォームでウィザードレイアウトのコアコンポーネントを使用して、ウィザードレイアウトを作成できます。
...
アダプティブフォームウィザードレイアウトコアコンポーネントは、コアコンポーネント 2.0.4 の一部として、2023年2月にリリースされました。 次の表に、サポートされているすべてのバージョン、AEM の互換性、対応するドキュメントへのリンクを示します。
...
繰り返しオプションを使用すると、ウィザードとその子コンポーネントのレプリケーション、最小繰り返し回数と最大繰り返し回数の定義、フォーム内の類似セクションのレプリケーションを簡単に行うことができます。 ウィザードコンポーネントを操作してその設定にアクセスする際には、次のオプションが表示されます。
