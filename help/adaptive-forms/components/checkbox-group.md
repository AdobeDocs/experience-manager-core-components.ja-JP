...
|---|---|---|
| v2 | リリース 3<br>[0.142](/help/adaptive-forms/version.md) 以降と互換性あり | |
...
## チェックボックスグループ（v2）のフィールドセットおよび凡例のサポート
...
アダプティブ Forms コアコンポーネントは、セマンティック `<fieldset>` と `<legend>` HTML要素を使用して、チェックボックスグループのアクセシビリティを向上させます。 これらの要素を使用すると、スクリーンリーダーでフォーム構造が正確に解釈され、グループラベルとそのオプションの間の関係が伝えられます。

チェックボックスグループ（v2）コンポーネントが `<fieldset>` 要素内でレンダリングされ、グループラベルが最初の子として `<legend>` にラップされます。 この構造により、スクリーンリーダーが個々のチェックボックスオプションを読み取る前にグループラベルを読み上げるので、支援テクノロジーを使用しているユーザーが各オプションのコンテキストと目的を理解しやすくなります。
