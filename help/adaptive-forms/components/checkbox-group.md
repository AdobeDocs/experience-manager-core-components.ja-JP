...
アダプティブフォームのチェックボックスグループコアコンポーネントは、Cloud Service 向けコアコンポーネント 2.0.4 および AEM 6.5.16.0 Forms向けコアコンポーネント 1.1.12 以降の一部として、2023年2月にリリースされました。 次の表に、サポートされているすべてのバージョン、AEM の互換性、対応するドキュメントへのリンクを示します。

| コンポーネントのバージョン | AEM as a Cloud Service | AEM 6.5.16.0 Forms 以降 |
|---|---|---|
| v2 | <br>[リリース 3.0.142](/help/adaptive-forms/version.md) 以降と互換性あり | |
...
>[!NOTE]
>
> チェックボックスグループコアコンポーネントの v1 バージョンでは、WCAG 2.1 がサポートされており、ユーザビリティと読みやすさのベストプラクティスに従って、コンテンツのアクセシビリティ向上に重点を置いています。 v2 バージョンは WCAG 2.2 をサポートしており、さらに一歩進んだ基準です。関連するフィールドの論理的かつ意味のあるグループ化を通じて、明確で予測可能かつ一貫性のあるアクセシビリティを重視しています。これは、包括的なフォームデザインの重要な要件となっています。
...
## チェックボックスグループ（v2）のフィールドセットと凡例のサポート
...
アダプティブフォームのコアコンポーネントは、セマンティック `<fieldset>` と `<legend>` HTML 要素を使用して、チェックボックスグループのアクセシビリティを向上させます。 これらの要素により、スクリーンリーダーはフォーム構造を正確に解釈し、グループラベルとオプション間の関係を伝えることができます。

チェックボックスグループ（v2）コンポーネントは、`<fieldset>` 要素内でレンダリングされ、グループラベルは `<legend>` で囲まれ、最初の子として表示されます。 この構造により、スクリーンリーダーは、個々のチェックボックスオプションを読む前にグループラベルを通知できるので、支援技術を使用するユーザーは各オプションのコンテキストと目的をより簡単に理解できます。

![チェックボックスフィールドセット](/help/adaptive-forms/assets/checkbox-fieldset.png)
