...
- **エラーメッセージの書式設定**  – このオプションを使用すると、入力した日付が正しい形式でない場合に、画面に表示されるメッセージを入力できます。
...
2. 既存のポリシーをとして選択 `datepicker-default-policy` ドロップダウンメニューから。

   ![日付選択テンプレートポリシー](/help/adaptive-forms/assets/date-picker-template-policy.png)
