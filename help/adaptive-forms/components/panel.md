...
アダプティブ Formsのパネルコンテナコアコンポーネントは、コアコンポーネント 2.0.4 の一部として 2023 年 2 月にリリースされました。 次の表に、サポートされているすべてのバージョン、AEM の互換性、対応するドキュメントへのリンクを示します。
...
- **セクション見出しの下のグループフィールド**：関連するフォームフィールドを共通のセクション見出しの下に整理するには、このオプションを選択します。 このグループ化により、スクリーンリーダーなどの支援テクノロジーで、グループ化されたフィールドを単一の包括的なユニットとして識別して読み上げることができ、すべてのユーザーに対するフォームのアクセシビリティとナビゲーションが向上します。

  ![&#x200B; パネルグループフィールド &#x200B;](/help/adaptive-forms/assets/panel-fieldset.png)
...
繰り返しオプションを使用すると、パネルコンテナとその子コンポーネントのレプリケーション、最小繰り返し回数と最大繰り返し回数の定義、フォーム内の類似セクションのレプリケーションを簡単に行うことができます。 パネルコンテナコンポーネントを操作してその設定にアクセスする際には、次のオプションが表示されます。
...
デザインダイアログでは、パネルコンポーネントの CSS スタイルを定義および管理できます。
