---
title: エクスペリエンスフラグメントのコンポーネント（v1）
...
# エクスペリエンスフラグメントのコンポーネント（v1） {#experience-fragment-component}
...
エクスペリエンスフラグメントコンポーネントをテンプレートに追加する場合、エクスペリエンスフラグメントがローカライズされていれば、このコンポーネントは自動的にローカライズされます。したがって、ページ上でレンダリングされるものは、明示的に選択したコンポーネントと異なる場合があります。[詳しくは &#x200B;](#example) 上記の例を参照してください。
